...
class SimpleEnginner(Role):
    # 定义一个类属性
    name: str = "dev-itdabai"
    # 定义一个类属性profile，描述该角色的信息
    profile: str = "code developer,write perfect code without debug, run code for test purpose"
    # 定义构造函数，接受任意数量的关键字参数
    def __init__(self, **kwargs):
        super().__init__(**kwargs)  # 执行父类Role的构造函数
        self.set_actions([SimpleWriteCode,SimpleRunCode])  # 为该角色配置拥有的动作/技能
...
async def main():
    msg = "wirte a python function that read and return excel content"
    role = SimpleEnginner()
    logger.info(msg)
